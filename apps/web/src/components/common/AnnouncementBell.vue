<template>
  <div>
    ...
    <button
      @click="openModal"
      class="notif-bell relative flex h-9 w-9 items-center justify-center rounded-lg text-gray-600 transition-all hover:bg-gray-100 hover:scale-105 dark:text-gray-400 dark:hover:bg-dark-800"
      :class="[
        { 'text-blue-600 dark:text-blue-400': unreadCount > 0 },
        props.labeled ? 'iconbtn iconbtn--labeled' : ''
      ]"
      :aria-label="t('announcements.title')"
    >
      <Icon name="bell" :size="props.labeled ? 'sm' : 'md'" />
      <span v-if="props.labeled" class="notif-bell__label">{{ notifyLabel }}</span>
      <!-- 未读红点 -->
      <span
        v-if="unreadCount > 0"
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div
          v-if="isModalOpen"
          class="mexion-float-overlay mexion-announcement-overlay fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-gradient-to-br from-black/70 via-black/60 to-black/70 p-4 pt-[8vh] backdrop-blur-md"
          @click="closeModal"
        >
          <div
            class="mexion-float-surface mexion-announcement-surface w-full max-w-[620px] overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Gradient -->
            <div class="mexion-float-header mexion-announcement-header relative overflow-hidden border-b border-gray-100/80 bg-gradient-to-br from-blue-50/50 to-indigo-50/30 px-6 py-5 dark:border-dark-700/50 dark:from-blue-900/10 dark:to-indigo-900/5">
            ...
            <!-- Body -->
            <div class="mexion-float-body mexion-announcement-body max-h-[65vh] overflow-y-auto">
              <!-- Loading -->
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div
          ...
          class="mexion-float-overlay mexion-announcement-overlay fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-gradient-to-br from-black/70 via-black/60 to-black/70 p-4 pt-[6vh] backdrop-blur-md"
          @click="closeDetail"
        >
          <div
            class="mexion-float-surface mexion-announcement-surface mexion-announcement-detail w-full max-w-[780px] overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/5 dark:bg-dark-800 dark:ring-white/10"
            @click.stop
          >
            <!-- Header with Decorative Elements -->
            <div class="mexion-float-header mexion-announcement-header relative overflow-hidden border-b border-gray-100 bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-purple-50/30 px-8 py-6 dark:border-dark-700 dark:from-blue-900/20 dark:via-indigo-900/10 dark:to-purple-900/5">
            ...
            <div class="mexion-float-body mexion-announcement-body max-h-[60vh] overflow-y-auto bg-white px-8 py-8 dark:bg-dark-800">
            ...
            <!-- Footer with Actions -->
            <div class="mexion-float-footer mexion-announcement-footer border-t border-gray-100 bg-gray-50/50 px-8 py-5 dark:border-dark-700 dark:bg-dark-900/30">
...
const notifyLabel = computed(() => (locale.value.startsWith('zh') ? '通知' : 'Notify'))
