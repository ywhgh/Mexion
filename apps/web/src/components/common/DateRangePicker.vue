<template>
  <div class="relative" ref="containerRef">
    ...
    <!-- Render at body level so cards with overflow hidden cannot clip the menu. -->
    <Teleport to="body">
...
const DROPDOWN_PREFERRED_WIDTH = 320
const DROPDOWN_ESTIMATED_HEIGHT = 263
...
const dropdownStyle = computed<CSSProperties>(() => {
  ...
  const width = Math.min(DROPDOWN_PREFERRED_WIDTH, availableWidth)
  ...
  const availableHeight = Math.max(
    ...
    dropdownPosition.value === 'top'
      ? rect.top - viewport.top - DROPDOWN_GAP - VIEWPORT_GUTTER
      : viewportBottom - rect.bottom - DROPDOWN_GAP - VIEWPORT_GUTTER,
  )
  const naturalHeight =
  ...
  nextTick(() => {
    ...
    const spaceBelow = Math.max(
      ...
      viewport.top + viewport.height -
        triggerRect.value.bottom -
        DROPDOWN_GAP -
        VIEWPORT_GUTTER,
    )
    const spaceAbove = Math.max(
      0,
      triggerRect.value.top - viewport.top - DROPDOWN_GAP - VIEWPORT_GUTTER,
    )
...
@media (max-width: 420px) {
  ...
  .date-picker-separator :deep(svg) {
