  ...
  return {
    ...
    useI18n: () => ({
      t: (key: string, params?: Record<string, string>) => {
        if (key === 'profile.memberId') return 'Member ID'
...
describe('ProfileInfoCard', () => {
  ...
  it('renders the approved overview hero and two-column content shell', () => {
    ...
    expect(wrapper.get('[data-testid="profile-overview-metric-member-id"]').text()).toContain('Member ID')
    expect(wrapper.get('[data-testid="profile-overview-metric-member-id"]').text()).toContain('5')
