...
interface BatchImageError extends Error {
  code?: string | number
  status?: number
  requestId?: string
}
...
async function parseBatchImageError(response: Response): Promise<Error> {
  try {
    ...
    const error: BatchImageError = new Error(message)
    error.code = body?.error?.code || response.status
    error.status = response.status
    error.requestId = response.headers.get('X-Request-Id') || ''
    return error
  } catch {
    const error: BatchImageError = new Error(response.statusText || `HTTP ${response.status}`)
