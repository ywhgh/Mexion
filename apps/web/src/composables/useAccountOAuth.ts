...
export function useAccountOAuth() {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      error.value = (err as ApiErrorLike).response?.data?.detail || 'Failed to generate auth URL'
  ...
  const exchangeAuthCode = async (
    ...
    try {
      ...
      error.value = (err as ApiErrorLike).response?.data?.detail || 'Failed to exchange auth code'
      appStore.showError(error.value)
      return null
    } finally {
      loading.value = false
  ...
  const cookieAuth = async (
    ...
    try {
      ...
      error.value = (err as ApiErrorLike).response?.data?.detail || 'Cookie authorization failed'
      return null
    } finally {
      loading.value = false
    }
