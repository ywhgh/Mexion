...
import type { GeminiOAuthCapabilities, GeminiAuthUrlRequest, GeminiExchangeCodeRequest } from '@/api/admin/gemini'
...
export function useGeminiOAuth() {
  ...
  const generateAuthUrl = async (
    ...
    try {
      ...
      if (oauthType) payload.oauth_type = oauthType as GeminiAuthUrlRequest['oauth_type']
      ...
      error.value = (err as ApiErrorLike).response?.data?.detail || t('admin.accounts.oauth.gemini.failedToGenerateUrl')
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      if (params.oauthType) payload.oauth_type = params.oauthType as GeminiExchangeCodeRequest['oauth_type']
