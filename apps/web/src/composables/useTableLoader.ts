 ...
 * 统一处理分页、筛选、搜索防抖和请求取消
 */
export function useTableLoader<T, P extends object>(options: TableLoaderOptions<T, P>) {
  ...
  const isAbortError = (error: unknown): boolean => {
    const e = error as { name?: string; code?: string } | null | undefined
    return e?.name === 'AbortError' || e?.code === 'ERR_CANCELED' || e?.name === 'CanceledError'
  }
