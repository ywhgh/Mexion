...
import type { AntigravityTokenInfo, AntigravityAuthUrlRequest, AntigravityExchangeCodeRequest } from '@/api/admin/antigravity'
...
export function useAntigravityOAuth() {
  ...
  const generateAuthUrl = async (proxyId: number | null | undefined): Promise<boolean> => {
    ...
    try {
      ...
      error.value =
        (err as ApiErrorLike).response?.data?.detail || t('admin.accounts.oauth.antigravity.failedToGenerateUrl')
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      error.value =
        (err as ApiErrorLike).response?.data?.detail || t('admin.accounts.oauth.antigravity.failedToExchangeCode')
      appStore.showError(error.value)
      return null
    } finally {
      loading.value = false
  ...
  const validateRefreshToken = async (
    ...
    try {
      ...
      error.value =
        (err as ApiErrorLike).response?.data?.detail || t('admin.accounts.oauth.antigravity.failedToValidateRT')
