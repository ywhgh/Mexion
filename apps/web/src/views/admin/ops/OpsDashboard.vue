<template>
  <component :is="isFullscreen ? 'div' : AppLayout" :class="isFullscreen ? 'flex min-h-screen flex-col justify-center bg-gray-50 dark:bg-dark-950' : ''">
    <div :class="[isFullscreen ? 'p-4 md:p-6 mexion-ops-surface--fullscreen' : '', 'mexion-ops-surface space-y-6 pb-12']">
      <div
        v-if="errorMessage"
        class="mexion-ops-notice rounded-2xl bg-red-50 p-4 text-sm text-red-600 dark:bg-red-900/20 dark:text-red-400"
      >
        {{ errorMessage }}
      </div>
      ...
      <div v-if="opsEnabled && !(loading && !hasLoadedOnce)" class="mexion-ops-plate grid grid-cols-1 gap-6 lg:grid-cols-4">
        <div class="mexion-ops-plate-cell min-h-[360px] lg:col-span-1">
          <OpsConcurrencyCard :platform-filter="platform" :group-id-filter="groupId" :refresh-token="dashboardRefreshToken" />
        </div>
        <div class="mexion-ops-plate-cell h-[360px] lg:col-span-1">
        ...
        <div class="mexion-ops-plate-cell h-[360px] lg:col-span-2">
      ...
      <div v-if="opsEnabled && !(loading && !hasLoadedOnce)" class="mexion-ops-plate mexion-ops-plate--analysis grid grid-cols-1 gap-6 md:grid-cols-3">
      ...
      <div v-if="opsEnabled && showOpenAITokenStats && !(loading && !hasLoadedOnce)" class="mexion-ops-section grid grid-cols-1 gap-6">
