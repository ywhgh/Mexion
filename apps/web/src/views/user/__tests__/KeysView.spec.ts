...
const MexionKeysChromeStub = {
  name: 'MexionKeysChrome',
  props: [
    'apiKeys', 'groups', 'loading', 'total', 'search', 'groupId', 'status',
    'selectedKeyId', 'copiedKeyId', 'apiBaseUrl', 'customEndpoints',
    'toggleableColumns', 'visibleColumnKeys', 'sortBy', 'sortOrder',
  ],
  emits: [
    'update:search', 'filter-search', 'filter-group', 'filter-status',
    'toggle-column', 'sort',
  ],
  template: `
    <div>
      <div data-test="columns">{{ visibleColumnKeys.join(',') }}</div>
      <div data-test="columns-meta">{{ JSON.stringify(toggleableColumns.map((column) => ({ key: column.key, sortable: !!column.sortable }))) }}</div>
      <button type="button" title="Column Settings">Column Settings</button>
      <button
        v-for="column in toggleableColumns"
        :key="column.key"
        type="button"
        :data-test="'toggle-column-' + column.key"
        @click="$emit('toggle-column', column.key)"
      >{{ column.label }}</button>
      <button
        type="button"
        data-test="sort-current-concurrency"
        @click="$emit('sort', 'current_concurrency', 'asc')"
      >Sort Current Concurrency</button>
      <div v-if="apiKeys[0]" data-test="current-concurrency">{{ apiKeys[0].current_concurrency }}</div>
      <div
        v-if="apiKeys[0] && visibleColumnKeys.includes('last_used_ip')"
        data-test="last-used-ip"
      >{{ apiKeys[0].last_used_ip }}</div>
  ...
  const wrapper = mount(KeysView, {
    global: {
      stubs: {
        ...
        MexionKeysChrome: MexionKeysChromeStub,
...
describe('user KeysView column settings', () => {
  ...
  it('keeps filters and selected page size when sorting by current concurrency', async () => {
    ...
    const chrome = wrapper.findComponent({ name: 'MexionKeysChrome' })
    await chrome.vm.$emit('update:search', 'target')
    await chrome.vm.$emit('filter-search')
    await flushPromises()

    await chrome.vm.$emit('filter-group', 42)
    await flushPromises()
    await chrome.vm.$emit('filter-status', 'active')
    await flushPromises()
    listKeys.mockClear()
