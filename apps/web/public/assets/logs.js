...
/* ───────── DATA + API ADAPTERS ───────── */
MexionI18n.register({
  en: {'logs.stream.col.tokens':'Tokens · Cache','logs.detail.meta.cache':'Cache hit','logs.detail.meta.trace':'Status / channel','logs.empty.title':'No logs yet','logs.empty.sub':'Logs will appear here once you start making API calls.'},
  zh: {'logs.stream.col.tokens':'Token · 缓存','logs.detail.meta.cache':'缓存命中','logs.detail.meta.trace':'状态 / 渠道','logs.empty.title':'暂无日志','logs.empty.sub':'开始调用 API 后，日志将在此显示。'}
});
...
function usd(u){void UNIT;return num(u)}
...
function loadGroups(){return api('/groups/available').then(function(d){GROUPS={};G_ORDER=[];list(d,['groups','items']).forEach(function(g,i){var k=g.id!=null?String(g.id):(g.name||('group_'+i));var ratio=num(g.ratio!=null?g.ratio:(g.rateMultiplier!=null?g.rateMultiplier/100:g.rate_multiplier/100));if(!ratio)ratio=1;var ent={id:g.id,name:g.name||k,mult:ratio,color:COLORS[i%COLORS.length]};GROUPS[k]=ent;if(g.name)GROUPS[g.name]=ent;G_ORDER.push(k)});if(!G_ORDER.length){GROUPS.default={name:'default',mult:1,color:'var(--mute-2)'};G_ORDER.push('default')}}).catch(function(){GROUPS={default:{name:'default',mult:1,color:'var(--mute-2)'}};G_ORDER=['default']})}
function mapLog(r){r=r||{};var d=date(r.created_at||r.createdAt||r.ts), st=parseInt(r.status_code!=null?r.status_code:r.status,10);if(!Number.isFinite(st))st=200;var inp=num(r.input_tokens!=null?r.input_tokens:(r.inputTokens!=null?r.inputTokens:r.prompt_tokens));var out=num(r.output_tokens!=null?r.output_tokens:(r.outputTokens!=null?r.outputTokens:r.completion_tokens));var total=num(r.total_tokens!=null?r.total_tokens:(inp+out));if(!inp&&!out&&total)inp=total;var actual=num(r.actual_cost!=null?r.actual_cost:(r.cost!=null?r.cost:0));var g=r.group_id!=null?String(r.group_id):(r.group_name||'default');if(r.group_name&&!GROUPS[g])GROUPS[g]={id:r.group_id,name:r.group_name,mult:1,color:gcolor(g)};var model=r.model||r.model_name||'—';var prov=r.provider||provider(model);var rawId=r.id!=null?r.id:Math.random().toString(36).slice(2);var req=r.request_id||r.requestId||('usage_'+rawId);var failed=st>=400;var keyId=r.api_key_id!=null?String(r.api_key_id):(r.key_id!=null?String(r.key_id):'');return{id:'log_'+rawId,rawId:rawId,reqId:req,t:hms(d),ts:d.getTime(),_date:ymd(d),method:r.method||'POST',path:r.path||'/v1',model:model,provider:prov,prov:prov,g:g,groupId:r.group_id,channelId:r.channel_id,key:keyId,keyName:r.api_key_name||r.key_name||keyId,status:st,st:failed?'err':'ok',finish:failed?String(st):'ok',errorCode:r.error_code||r.error||'',tok:{i:inp,o:out,c:0,total:total||inp+out},lat:num(r.duration_ms!=null?r.duration_ms:(r.durationMs!=null?r.durationMs:r.duration)),ttfb:num(r.ttft_ms!=null?r.ttft_ms:(r.ttftMs!=null?r.ttftMs:0)),costUnits:actual,cost:actual,bodyHash:r.body_hash||r.bodyHash||'',bodyLength:num(r.body_length!=null?r.body_length:r.bodyLength)}}
function fetchLogs(){var qs=new URLSearchParams({page:'1',page_size:String(LIMIT)});if(FILTER.key)qs.set('api_key_id',FILTER.key);if(FILTER.model)qs.set('model',FILTER.model);return api('/usage?'+qs.toString()).then(function(d){ALL_LOGS=list(d,['items','logs']).map(mapLog);return ALL_LOGS}).catch(function(){ALL_LOGS=[];return ALL_LOGS})}
function fetchUsage(){return api('/usage/dashboard/stats').then(function(d){USAGE=d||{};return USAGE}).catch(function(){USAGE={};return USAGE})}
function fetchKeys(){return api('/keys?page=1&page_size=50').then(function(d){KEYS=list(d,['items','keys']).map(function(k,i){var used=num(k.used_quota!=null?k.used_quota:(k.quotaUsed!=null?k.quotaUsed:k.quota_used));var lim=num(k.quota!=null?k.quota:(k.quotaLimit!=null?k.quotaLimit:k.quota_limit));var id=k.id!=null?String(k.id):'';return{id:k.id,name:k.name||('Key '+(i+1)),key:id,prefix:k.key||k.prefix||k.keyPrefix||id,usd:used,remainUsd:lim>0?Math.max(0,lim-used):null,unlimited:!(lim>0),active:(k.status||'active')==='active',lastUsed:k.last_used_at||k.lastUsedAt||k.accessed_time||null,color:COLORS[i%COLORS.length]}});return KEYS}).catch(function(){KEYS=[];return KEYS})}
function updateKPI(){var calls=num(USAGE.total_requests), err=ALL_LOGS.filter(function(l){return l.status>=400}).length, lat=num(USAGE.average_duration_ms), cost=num(USAGE.total_actual_cost);var c=E('kpiCalls');if(c)c.textContent=fmt(calls);var e=E('kpiErr');if(e)e.textContent=fmt(err);var ed=E('kpiErrDelta');if(ed)ed.textContent=calls?((err/Math.max(calls,1)*100).toFixed(2)+(en()?'% error rate':'% 错误率')):'';var l=E('kpiLat');if(l)l.textContent=lat?dur(lat):'—';var ce=E('kpiCost');if(ce)ce.textContent=cost?money(cost):'—';var sub=E('heroSub');if(sub)sub.innerHTML=en()?('Total <strong>'+fmt(calls)+'</strong> calls · today <strong>'+fmt(num(USAGE.today_requests))+'</strong> · spend <strong>'+money(cost)+'</strong> · latency <strong>'+dur(lat)+'</strong>'):('累计 <strong>'+fmt(calls)+'</strong> 次调用 · 今日 <strong>'+fmt(num(USAGE.today_requests))+'</strong> 次 · 总开销 <strong>'+money(cost)+'</strong> · 平均耗时 <strong>'+dur(lat)+'</strong>')}
...
function tokenView(l){var second='<span class="log__tok-cache" style="color:var(--mute-3);">—</span>';if(!l.tok.i&&!l.tok.o)return'<span class="log__tok is-empty"><span>—</span>'+second+'</span>';return'<span class="log__tok"><span class="log__tok-row"><span class="log__tok-arr is-in">↓</span><b>'+tok(l.tok.i)+'</b><span class="log__tok-arr is-out">↑</span><b>'+tok(l.tok.o)+'</b></span>'+second+'</span>'}
...
function makePayload(l){var note=en()?'For safety, request and response bodies are not stored.':'安全考虑，请求内容不记录原文。';var meta={requestId:l.reqId,model:l.model,provider:l.provider,method:l.method,path:l.path,apiKey:l.keyName||l.key||null,status:l.status,inputTokens:l.tok.i,outputTokens:l.tok.o,totalTokens:l.tok.total||l.tok.i+l.tok.o,durationMs:l.lat,ttftMs:l.ttfb||null,actualCostUsd:Number(l.cost.toFixed(6)),cacheHit:null,errorCode:l.errorCode||null,note:note};return{req:json({requestId:l.reqId,method:l.method,path:l.path,model:l.model,bodyHash:l.bodyHash||null,bodyLength:l.bodyLength||0,note:note}),res:json(meta),hdr:esc('x-mexion-request-id: '+l.reqId+'\nx-mexion-api-key: '+(l.keyName||l.key||'-')+'\nstatus: '+l.status+'\nprovider: '+(l.provider||'-')),curl:'curl '+window.location.origin+'/v1/chat/completions \\\n  -H "Authorization: Bearer $MEXION_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '+JSON.stringify(JSON.stringify({model:l.model,messages:[{role:'user',content:'Hello'}],stream:false}))}}
...
document.addEventListener('DOMContentLoaded',function(){if(window.MexionAuth&&MexionAuth.guard)MexionAuth.guard();initPulseTooltip();wire();refreshFb();refresh(false);setInterval(function(){refresh(false)},30000);document.addEventListener('user-menu:signout',function(){if(window.MexionAuth)MexionAuth.logout()})});
