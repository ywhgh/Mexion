(function () {
  ...
  function translateMenu() {
    ...
    document.querySelectorAll('.user-menu__item').forEach(function(item) {
      ...
      if (href.indexOf('profile') !== -1) span.textContent = MexionI18n.t('shared.nav.profile');
      else if (href.indexOf('api-keys') !== -1) span.textContent = MexionI18n.t('shared.nav.apikeys');
      else if (action === 'signout') span.textContent = MexionI18n.t('shared.nav.signout');
    });
    document.querySelectorAll('.notif-bell').forEach(function(btn) {
      btn.setAttribute('title', MexionI18n.t('shared.topbar.notify'));
    });
    document.querySelectorAll('[aria-label="账户菜单"]').forEach(function(el) {
      el.setAttribute('aria-label', MexionI18n.t('shared.menu.account'));
    });
  }
  ...
  function injectCommunityBtn() {
    ...
    var _commLabel = (typeof MexionI18n !== 'undefined' && MexionI18n.lang === 'en') ? 'Community' : '社群';
    ...
    wrap.querySelector('#communityBtn').addEventListener('click', function() {
      ...
      popup.innerHTML =
        '<div style="background:var(--surface,#fff);border:1px solid var(--border,#E6E2D6);border-radius:14px;max-width:380px;width:100%;padding:28px 24px;box-shadow:0 24px 60px -16px rgba(20,18,14,0.3);animation:cpIn .25s ease-out;position:relative">' +
          ...
          '<div style="display:flex;flex-direction:column;gap:10px;margin-top:20px">' +
            ...
            '<a href="mailto:mexioncodeai@gmail.com" style="display:flex;align-items:center;gap:12px;padding:12px 14px;border:1px solid var(--border,#E6E2D6);border-radius:10px;text-decoration:none;color:var(--ink,#14140F);transition:all .15s"><span style="width:36px;height:36px;border-radius:9px;background:var(--ink,#14140F);display:flex;align-items:center;justify-content:center;flex-shrink:0"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="2" y="3.5" width="12" height="9" rx="1" stroke="#fff" stroke-width="1.3"/><path d="M2 4l6 4.5L14 4" stroke="#fff" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg></span><div style="flex:1"><div style="font-weight:500;font-size:14px">'+(l==='zh'?'邮件联系':'Email')+'</div><div style="font-size:12px;color:var(--mute-2,#94917F)">mexioncodeai@gmail.com</div></div><svg width="12" height="12" viewBox="0 0 12 12" fill="none" style="color:var(--mute-3)"><path d="M4.5 2L8.5 6 4.5 10" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg></a>' +
          '</div>' +
      ...
      if (wechatBtn) wechatBtn.addEventListener('click', function() {
        ...
        ov.innerHTML =
          '<div class="wc-card">' +
            '<div class="wc-card__head"><div class="wc-card__brand"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1l1.5.5L11 3l1 1.5.5 1.5V8l-.5 1.5L11 11l-1.5 1-1.5.5H6.5L5 12 3.5 11l-1-1.5L2 8V6.5L2.5 5 3.5 3.5 5 2.5 6.5 2z" stroke="currentColor" stroke-width="1.2"/></svg><span>Mexion</span></div><div class="wc-card__tag"><span class="wc-dot"></span>WeChat</div></div>' +
            '<div class="wc-card__body"><div class="wc-qr"><img src="/assets/wechat-group/qr.svg?v=3" alt="WeChat QR"></div></div>' +
            '<div class="wc-card__foot"><div class="wc-foot-left"><div class="wc-foot-title">'+(l==='zh'?'微信扫码入群':'Scan to Join')+'</div><div class="wc-foot-sub">'+(l==='zh'?'Mexion 用户交流群':'Mexion Community')+'</div></div><button class="wc-close" onclick="this.closest(\'.wc-overlay\').remove()">'+(l==='zh'?'关闭':'Close')+'</button></div>' +
          '</div>';
  ...
  function boot() {
    ...
    translateMenu();
    injectCommunityBtn();
    if (typeof MexionI18n !== 'undefined') MexionI18n.onChange(translateMenu);
  }
